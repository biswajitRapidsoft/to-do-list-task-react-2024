...
export default function ToDo(props) {
    return (
        <div className="accordion-item mb-1" key={props.todo.id}>
            <h2 className="accordion-header">
            <button className="accordion-button collapsed bg-secondary text-white rounded fs-5" type="button" data-bs-toggle="collapse" data-bs-target={`#flush-collapse-${props.todo.id}`} aria-expanded="false" aria-controls={`#flush-collapse-${props.todo.id}`}>
                Task @{props.todo.id}
            </button>
            </h2>
            <div id={`flush-collapse-${props.todo.id}`} className="accordion-collapse collapse">
                <div className="accordion-body d-flex justify-content-between align-items-center bg-light">
                    <p className={`my-auto ${props.todo.completed ? 'text-decoration-line-through text-body-tertiary' : ''}`} style={{width: '60%'}}>Task: {props.todo.text} | To be done at {`${props.todo.time}`} on {`${props.todo.date}`}</p>
